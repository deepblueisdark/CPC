...
def ler_configuracao(regiao="AMS", arquivo_config="cpc.config"):
    ...
    if regiao not in config:
        raise ValueError(f"[ERRO] Região '{regiao}' não encontrada em {arquivo_config}")
    ...
    shapefile_dir = config["DEFAULT"].get("shapefile_dir", "./SHAPES/")
    shapefile_padrao = config["DEFAULT"].get("shapefile_padrao", "*.shp")

    return caminho, prefixo, escala_cores, shapefile_dir, shapefile_padrao
...
def calcular_estatisticas_bacia(chuva, lon, lat, shapefile_path):
    ...
    for idx, row in gdf.iterrows():
        try:
            ...
            resultados.append([shapefile_path, idx, f"shape_{idx}",
...
def processar_chuva_cpc(data_ini, data_fim, regiao="AMS", saida="chuva_diaria_CPC.xlsx",
                        mapas=False, classificacao=False, shapefile_dir=None, shapefile_padrao=None):
    repo, prefixo, escala, dir_cfg, padrao_cfg = ler_configuracao(regiao, CONFIG_FILE)
    SHAPEFILE_DIR_FINAL = shapefile_dir if shapefile_dir else dir_cfg
    SHAPEFILE_PADRAO_FINAL = shapefile_padrao if shapefile_padrao else padrao_cfg
    ...
    for dia in datas:
        ...
        shapefiles = sorted(glob(os.path.join(SHAPEFILE_DIR_FINAL, SHAPEFILE_PADRAO_FINAL)))
...
def parse_data_flexivel(data_str):
    formatos_possiveis = ["%Y-%m-%d", "%d/%m/%Y", "%d-%m-%Y", "%Y/%m/%d", "%Y.%m.%d", "%d.%m.%Y", "%Y%m%d", "%d%m%Y"]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--classificacao", action="store_true", help="Gera mapas de classificação por ponto (scatter)")
    parser.add_argument("--shapefile", default=None, help="Diretório com os shapefiles (sobrepõe o valor do cpc.config)")
    parser.add_argument("--padrao", default=None, help="Padrão dos shapefiles (ex: 'SAE*.shp'), sobrepõe o valor do cpc.config")
    ...
    processar_chuva_cpc(
        ...
        shapefile_dir=args.shapefile,
        shapefile_padrao=args.padrao
    )
